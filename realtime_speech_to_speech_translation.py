"""
This script performs real-time speech-to-speech translation using Azure Cognitive Services.
It captures speech from the default microphone, 
translates it from a source language to a target language,
and then synthesizes the translated text into speech using the default speaker.
"""
import logging
import os
import time
...
def main():
    ...
    tts_config = speechsdk.SpeechConfig(subscription=tts_key, region=tts_region)
    tts_config.speech_synthesis_voice_name = (
        "fr-FR-VivienneMultilingualNeural"  # Voice matching the target language
    )
    ...
    synthesizer = speechsdk.SpeechSynthesizer(
        speech_config=tts_config, audio_config=audio_output_config
    )
